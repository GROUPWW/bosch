...
VARIANTS = [
    ("c-i_sim_only",   "stageB_best", 0.0, False, -1),
...
FAM_CFG = {
    "zmin": {"lr": 1e-5, "file": "Bosch_zmin_select_aug_v2.xlsx", "split": "fixed8", "ss": 2026},
...
def main():
    out_dir = os.path.join(_HERE, "runs_stageC_v14_v14_clean")
    ...
    summary = os.path.join(out_dir, "summary_v14.csv")
    ...
    for fam, fc in FAM_CFG.items():
        ...
        for tag, init, l2sp, prog, epochs in VARIANTS:
            name = f"v14_{tag}_{fam}" if epochs != -1 else f"v14b_c-i_true0_{fam}"
            exp = sc.ExpCfg(
    ...
    piv.index = [e.replace("v14_", "").rsplit("_", 1)[0] for e in piv.index]
    print("\n[v7 汇总 R² 矩阵]")
