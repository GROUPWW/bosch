#!/usr/bin/env python3
"""v18 · R2-3b StageC 侧补全：gru/lstm/tcn/ar_mlp × {zmin,h1,d1,w} 迁移（干净数据，钻石配置）。"""
...
HP = "lr0p001_wd0p0001_do0p1_dm256_L2_hb0p1_stA_t_full"
EXCLUDE = {"B143", "B166"}

FAMS = {
    # fam: (file, split_tag, ss, lr, l2sp, br, epochs)
...
BACKBONES = ["gru", "lstm", "tcn", "ar_mlp"]


def main():
    out_dir = os.path.join(_HERE, "runs_stageC_v18_backbone_transfer")
    sc.ensure_dir(out_dir)
    device = su.get_default_device()
    print(f"[v18] device={device}  {len(BACKBONES)} backbones × {len(FAMS)} fams")
    ...
    summary = os.path.join(out_dir, "summary_v18.csv")
    ...
    for fam, (file, split_tag, ss, lr, l2sp, br, epochs) in FAMS.items():
        ...
        for bb in BACKBONES:
            ckpt = os.path.join(ABL, f"{bb}_{HP}_{fam}_s3", "best.pth")
            assert os.path.exists(ckpt), ckpt
            name = f"v18_{bb}_{fam}_ep{epochs}"
            exp = sc.ExpCfg(
            ...
            run_args = argparse.Namespace(
                ...
                recipe_aug_mode="time", model_type=bb, batch=4,
            ...
            sc.run_datasets(datasets=[sp], raw=raw, families_eval=[fam],
                            experiments=[exp], stageB_best={fam: ckpt}, args=run_args,
                            device=device, summary_all=summary, header_fields=hf,
                            done_keys=dk, epochs=epochs, patience=99999,
                            tag="v18", key_recipes=[])
            r = pd.read_csv(summary).iloc[-1]
            print(f"[v18 RESULT] {bb:7s} {fam:5s}: R2={r.r2:+.4f} best_ep={r.best_epoch}", flush=True)
